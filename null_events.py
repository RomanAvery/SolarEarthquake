...
for index, row in earthquake.iterrows():
    #Get date for this row and next row
    date = row['date']
    start = dp.parse(date)
    #Break loop once it reaches the last row
    try:
        other_date = earthquake['date'].loc[index+1]
    except:
        break
    end = dp.parse(other_date)
    
    #Find whether time difference between dates are sufficiently large
    delta = end - start
    minimum = pd.Timedelta('2 days')
    if delta > minimum:
        #Create start and end date far away from earthquake events
        end_date = end - datetime.timedelta(days=delta.days/2)
...
out_data.to_csv('data/ambient_set.tsv', sep='\t')